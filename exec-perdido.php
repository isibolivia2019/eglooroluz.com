...
$listaProducto = $modelo->listaProductoPerdidos($datos);
$c = 0;
for($i=0 ; $i<sizeof($listaProducto) ; $i++){
        ...
        if(sizeof($listaInventario) > 0){
            $datos = array($listaInventario[0]['cod_inventario'], $listaProducto[$i]['cod_producto_perdido']);
            $modelo = modelo('ProductoPerdido');
            $res = $modelo->actualizaCodInventarioProductoPerdidos($datos);
        }
