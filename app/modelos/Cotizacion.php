...
class Cotizacion{
	...
	public function listaCarritoCotizacion($datos){
		$sql = "SELECT codigo, carrito_cotizacion.cod_inventario, cod_item_producto, nombre_producto, imagen_producto, precio_sugerido_venta, cantidad, descuento, total FROM carrito_cotizacion, inventario, producto WHERE carrito_cotizacion.cod_inventario = inventario.cod_inventario and inventario.cod_producto = producto.cod_producto and carrito_cotizacion.cod_sucursal = ?";
	...
	public function agregarCotizacion($datos){
		$sql = "INSERT INTO cotizacion(cod_sucursal, cod_inventario, cant_producto, descuento_porcentaje_venta_producto, total_unitario, nro_orden, empresa, personal, fecha, hora, cod_usuario) VALUES(?,?,?,?,?,?,?,?,?,?,?);";
